...
import {
  masterStatusBayar,
  masterStatusOrder,
} from 'src/modules/cms/order_resto/order_resto.dto';
import { paymentMethodEntity } from './payment_method.entity';

@Table({
  tableName: 'order_resto',
  timestamps: true,
...
export class orderRestoEntity extends Model<orderRestoEntity> {
  ...
  get status_order_name(): string {
    const status = masterStatusOrder.find(
      (s) => s.status_code === this.status_order,
    );
    return status ? status.status_name : 'Unknown Status';
  }

  get status_bayar_name(): string {
    const status = masterStatusBayar.find(
      (s) => s.status_code === this.status_bayar,
    );
    return status ? status.status_name : 'Unknown Status';
  }
  ...
  toJSON() {
    ...
    return {
      ...attributes,
      status_order_name: this.status_order_name,
      status_bayar_name: this.status_bayar_name,
    };
  }
}
