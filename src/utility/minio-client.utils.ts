...
export class MinioClientService {
  ...
  public async upload(file: BufferedFile, baseBucket: string) {
    if (
      !(
        ...
        file.mimetype.includes('vnd.android.package-archive')
      )
    ) {
  ...
  public async upload_file(file: BufferedFile, baseBucket: string, fileName) {
    const metaData = {
      'Content-Type': file.mimetype,
    };
    ...
    return {
      url: `https://minio.unioniptv.id/${baseBucket}/${fileName}`,
      fileName: fileName,
      ImageTypeNotSupported: false,
    };
  }
}
