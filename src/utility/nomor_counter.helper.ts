...
export class generateNumber {
  ...
  async getNumbering(
    ...
    let pad = '00';
    let getbulan =
      pad.substring(0, pad.length - getbyDesc.bulan.length) + getbyDesc.bulan;
    ...
    // console.log(gettahun);
    // console.log(now_tahun);
    // console.log(parseInt(getbulan));
    // console.log(parseInt(now_bulan));
    if (gettahun == now_tahun && parseInt(getbulan) == parseInt(now_bulan)) {
      ...
      number =
        getkode + sethotelId + '-' + now_tahun_yy + now_bulan + '-' + ans;
