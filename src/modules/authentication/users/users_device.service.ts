...
export class UserDeviceService {
  ...
  async login(param: loginDeviceDto, req: any): Promise<any> {
    try {
      ...
      let cek_sess = await this._sessionDeviceEntity.findOne({
        where: { id_user_device: user.id_user_device },
      });
      if (cek_sess != null) {
  ...
  async insertUserRoom(param: createUserRoom, req: any): Promise<any> {
    try {
  ...
  async updateUserRoom(param: updateUserRoom, req: any): Promise<any> {
    try {
      let cekDuplicateUser = await this._users_deviceEntity.findOne({
        where: {
          username: param.username,
