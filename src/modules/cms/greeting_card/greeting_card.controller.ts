...
export class GreetingCardController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Return all greeting_card.',
    type: [greeting_card],
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Return a single greeting_card.',
    type: greeting_card,
  })
  @Get(':id_greeting_card')
  findOne(
    @Param('id_greeting_card') id_greeting_card: number,
  ...
  create(
    @Body() greeting_card: greeting_cardDtoInsert,
    @Req() req: Request,
  ): Promise<greeting_card> {
  ...
  @ApiResponse({
    status: 200,
    description: 'The greeting_card has been successfully updated.',
    type: greeting_card,
  })
  update(
    @Param('id_greeting_card') id_greeting_card: number,
    ...
    return this.greeting_cardService.update(
      id_greeting_card,
      greeting_card,
      req,
    );
  ...
  remove(@Param('id_greeting_card') id: number, @Req() req: Request) {
    return this.greeting_cardService.remove(id, req);
  }
  ...
  updateStatusActive(
    @Param('id_channel') id_channel: number,
    @Req() req: Request,
  ) {
    return this.greeting_cardService.updateStatusActive(id_channel, req);
  }
}
