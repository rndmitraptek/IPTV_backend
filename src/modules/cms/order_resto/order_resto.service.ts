...
export class OrderRestoService {
  ...
  findOne(id: number): Promise<any> {
    return this._orderRestoEntity.findOne({
      ...
      order: [[this.sequelize.col('detail.id_order_resto_detail'), 'desc']],
    });
  }
  ...
  async create(param: insertOrderResto, req: any): Promise<any> {
    ...
    try {
      ...
      let number = await this._generateNumber.getNumbering(
        'order_resto',
        transaction,
        req.user.id_hotel,
      );
      if (number) {
        let update = await this._generateNumber.updateCounter(
          'order_resto',
          number[1],
          transaction,
        );
      ...
      let insertHeader = await this._orderRestoEntity.create(
        _orderRestoEntity,
  ...
  async jenisPembayaranByHotel(req: any): Promise<any> {
    ...
    let getHotel = await this._iptv_feature.findOne({
  ...
  async pembayaran(param: pembayaranOrder, req: any): Promise<any> {
    let transaction = await this.sequelize.transaction();
    try {
      let getData = await this._orderRestoEntity.findOne({
  ...
  async verifyCallback(callbackData: any): Promise<any> {
    try {
      ...
      if (getData == null) {
        await this._MidtransService.logCallback(
          callbackData,
      ...
      if (getHotel.midtrans_server_key == null) {
        await this._MidtransService.logCallback(
          callbackData,
          'Serverkey tidak ditemukan',
          order_id,
        );
        throw 'Serverkey tidak ditemukan';
      }

      let isValid = await this._MidtransService.verifySignature(
        callbackData,
        getHotel.midtrans_server_key,
      );
      if (!isValid) {
        await this._MidtransService.logCallback(
          callbackData,
  ...
  async updateStatusOrder(param: updateStatusOrder, req: any): Promise<void> {
    ...
    let send = await this._AppGateway.handleMessageStatusOrder(cekData);
  }
}
