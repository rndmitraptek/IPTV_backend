...
import {
  apk_versionDtoInsert,
  apk_versionDtoInsertWithoutFile,
  secretKeyVersion,
  uploadFile,
} from './apk_version.dto';
...
export class ApkVersionController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Return a single apk_version.',
    type: apk_version,
  })
  @Get(':secretkey')
  ...
  @ApiOperation({
    summary:
      'tambah data apk_version tanpa file (file hanya nama hasil generate lewat presignedurl)',
  })
  @ApiResponse({
    status: 201,
    description: 'The user has been successfully created.',
  ...
  @Post('uploadFile')
  @ApiOperation({ summary: 'upload file iptv-config' })
  @ApiConsumes('multipart/form-data')
  @UseInterceptors(FileFieldsInterceptor([{ name: 'file', maxCount: 1 }]))
  async uploadFile(
  ...
  @ApiOperation({ summary: 'get file iptv-config' })
  getFile(@Param('filename') filename: string): Promise<any> {
    return this.minioClientService.generatePresignedUrlGet(
      'iptv-config',
      filename,
    );
  }
