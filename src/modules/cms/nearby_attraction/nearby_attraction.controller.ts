...
export class NearbyAttractionController {
  constructor(
    private readonly nearby_attractionService: NearbyAttractionService,
  ) {}
  ...
  @ApiResponse({
    status: 200,
    description: 'Return all nearby_attraction.',
  ...
  @ApiBearerAuth('access-token')
  @ApiOperation({
    summary: 'Menampilkan nearby_attraction by id nearby_attraction',
  })
  @ApiResponse({
    status: 200,
    description: 'Return a single nearby_attraction.',
    type: nearby_attraction,
  })
  @Get(':id_nearby_attraction')
  findOne(
    @Param('id_nearby_attraction') id_nearby_attraction: number,
  ...
  create(
    @Body() nearby_attraction: nearby_attractionDtoInsert,
    @Req() req: Request,
  ): Promise<nearby_attraction> {
  ...
  @ApiResponse({
    status: 200,
    description: 'The nearby_attraction has been successfully updated.',
    type: nearby_attraction,
  })
  update(
    @Param('id_nearby_attraction') id_nearby_attraction: number,
    ...
    return this.nearby_attractionService.update(
      id_nearby_attraction,
      nearby_attraction,
      req,
    );
  }
