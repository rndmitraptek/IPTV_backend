...
export class InfoRoomController {
    ...
    @ApiResponse({ status: 200, description: 'Return all info_room.', type: [info_room] })
    findAll(): Promise<info_room[]> {
        return this.info_roomService.findAll();
    }

    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth('access-token')
    @ApiOperation({ summary: 'Menampilkan info_room by id info_room' })
    @ApiResponse({ status: 200, description: 'Return a single info_room.', type: info_room })
    @Get(':id_info_room')
    ...
    @ApiResponse({ status: 200, description: 'The info_room has been successfully updated.', type: info_room })
    update(@Param('id_info_room') id_info_room: string, @Body() info_room: info_roomDtoInsert) {
        return this.info_roomService.update(id_info_room, info_room);
    }

    @ApiOperation({ summary: 'Delete data DtoInsert' })
    @ApiResponse({ status: 200, description: 'The DtoInsert has been successfully deleted.' })
    @Delete(':id_info_room')
    remove(@Param('id_info_room') id: number) {
        return this.info_roomService.remove(id);
    }
}
